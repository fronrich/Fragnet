...
/**
 * Credit to Swen Wenzel for this code
 * Modifed to create dns seed by Fronrich Puno
 */
...
// generate a random, english sounding dns hash
export const genDNSHash = (node, anonimity, customName) => {
  const length = 3;
  const seed = generateWords(length);
  ...
  switch (anonimity) {
    ...
    case 1:
      ...
      break;
    case 2:
      fdclName = generateWords(1);
      break;
  }
  fdclName = fdclName.replace(/[^a-zA-Z]/g, "").toLowerCase();

  // fdcl stands for 'fragnet debris cluster location'
  return `fdcl://${fdclName}.${node.toLowerCase()}${seed}`;
};

// gets the current ip of the node
export const getIP = () => {
